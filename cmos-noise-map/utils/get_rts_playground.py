...
def plot_get_rts(p, tol = 5, upper_q = 3, min_peak_sep = 10):
    ...
    #-------------Plotting-------------------
        ...
        ax.set_title(str(np.abs(sils[-1]-sils[-2]))+ "    "+ str(n_components), fontname = 'Libre Franklin')
        ...
        mpl.rcParams['axes.prop_cycle'] = mpl.cycler(color=["#4B88A2", "#BB0A21", "#252627", "#F39237", "#849324"]) 
        ...
        ax_plotx = fig.add_axes([left, bottom + height + spacing, width, 0.2], sharex= ax1)
        ...
        for i in np.arange(0, n_components):
            x = np.linspace(peaks[i][0] - 3*variances[i][0][0], peaks[i][0] + 3*variances[i][0][0], 100)
            ax_plotx.plot(x, scipy.stats.norm.pdf(x, peaks[i][0], variances[i][0][0]), label = f"Mean: {peaks[i][0]: 0.2f} Var: {variances[i][0][0]: 0.2f}", alpha=0.8)
...
def noise_distribution(data, gain=None):
    ...
    ax.hist(hist_data, density = True, bins=1000, range=[median-5*std, median+ 7* std], color='#252627')
    ...
    ax.set_title ("Distribution of Per Pixel Noise", fontname='Libre Franklin', fontsize=20)
