...
class plot_get_rts:
    def __init__(
        ...
        for row_no in np.arange(start_row, end_row):
            ...
            for im in images:
                data.append(
                    (im.data[row_no, start_column:end_column] + self.bzero)
                    * self.bscale
                )
        ...
        for p in self.pixels[0]:
            if np.std(p) > upper_quantile:
                ...
                if num_peaks == 2 or num_peaks == 3:
                    ...
                    gmm_y = np.exp(
                        fits[n_components - 1].score_samples(gmm_x.reshape(-1, 1))
                    )
                    fig, ax = plt.subplots()
...
class noise_distribution:
    ...
    def noise_distribution_plot(self, upper_quantile=0.90, bins=1000):
        ...
        ax.vlines(
            ...
            linestyle="--",
            alpha=0.8,
            label=f"{upper_quantile*100}th percentile = {upper_q: 0.2f}",
        )
        ax.legend()
        ax.set_title("Distribution of Per Pixel Noise", fontsize=20)
